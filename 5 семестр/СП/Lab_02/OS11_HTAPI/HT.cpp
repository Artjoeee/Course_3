...
namespace ht
{
	...
	void CALLBACK snapAsync(LPVOID prm, DWORD, DWORD)
	{
		...
		if (snap(htHandle))
		{
			cout << "-- spanshotAsync success" << std::endl;
		}
	}
		...
		const wchar_t* fileName)
	{
		...
		wstring mutexName = wstring(fileName) + L"_MUTEX";
		...
		wstring ownerEventName = wstring(fileName) + L"_OWNER";
		...
		const wchar_t* fileName)
	{
		...
		if (!ownerEvent) 
		{
			wprintf(L"Ошибка: событие не найдено (%s (OS11_START не работает))\n", ownerEventName.c_str());
			return NULL;
		}
		...
		const Element* element)              // элемент 
	{
		...
		cout << "\t\"keyLength\": " << element->keyLength << "," << endl;
		cout << "\t\"payload\": \"" << (char*)element->payload << "\"," << endl;
		cout << "\t\"payloadLength\": " << element->payloadLength << endl;
		cout << "}" << endl;
	}
	...
	int findFreeIndex(const HtHandle* htHandle, const Element* element)
	{
		...
		while (attempts < htHandle->capacity)
		{
			if (index >= htHandle->capacity)
			{
				index %= htHandle->capacity;
			}

			readFromMemory(htHandle, &foundElement, index);

			if (foundElement.keyLength == 0 || foundElement.payloadLength == 0)
			{
				return index;
			}
			else if (foundElement.keyLength != 0 &&
				memcmp(foundElement.key, element->key, element->keyLength) == 0)
			{
				return -1;
			}
	...
	int findIndex(const HtHandle* htHandle, const Element* element)
	{
		...
		while (attempts < htHandle->capacity)
		{
			...
			if (foundElement.keyLength == element->keyLength &&
				memcmp(foundElement.key, element->key, element->keyLength) == 0)
			{
				return index;
			}
