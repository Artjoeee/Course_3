...
int main() 
{
	...
	try 
	{
		...
		while (true) 
		{
			...
			{
				PutAnswerToClient("Error name", (sockaddr*)&clnt, &lc);
			}

			SOCKADDR_IN* addr = (SOCKADDR_IN*)&clnt;

			cout << "Порт клиента: " << addr->sin_port << endl;
			cout << "IP-адрес клиента: " << inet_ntoa(addr->sin_addr) << endl;
