...
int main()
{
	...
	try
	{
		...
		while (true) 
		{
			int lb = recvfrom(sS, ibuf, sizeof(ibuf), 0, (sockaddr*)&clnt, &lc);
			if (lb == SOCKET_ERROR) continue;

			std::string message(ibuf, lb);
			std::cout << "Получено: " << message << std::endl;

			std::string response = "ECHO: " + message;
			sendto(sS, response.c_str(), response.size(), 0, (sockaddr*)&clnt, lc);
		}
